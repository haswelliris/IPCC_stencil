...
//             }
            ...
            _mm256_storeu_ps(out_3 + j, reg_o_3);
            _mm256_storeu_ps(out_4 + j, reg_o_4);
        }

        // for (; j < width - 1; j++)
        { // 处理最后的
            int last_size = width - j;
            // unsigned int mask0[8] = {0.0};
            ...
            // _mm256_maskstore_ps(out_3 + j, r_mask, reg_o_3);
            // _mm256_maskstore_ps(out_4 + j, r_mask, reg_o_4);
        }
    }
    // 注意不要越界了
    // 处理剩下的
